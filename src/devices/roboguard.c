...
static int roboguard_ibis_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    data_t *data;
    uint8_t b[27]; // 24 for 192 bits, 27 for 216 bits
    ...
    // 216-24 = 192 bits
    if (bitbuffer->bits_per_row[0] != 216) // 216 bits = 27 bytes
    {
    ...
    // ffffff = 111111111111111111111111 24 high bits but inverted 000000 = 000000000000000000000000 24 low bits
    // We first check the preambe first 3 bytes 24 bits make sure they are all 0
    // p0:0, p1:0, p2:0
    if (b[0] != 0x00 || b[1] != 0x00 || b[2] != 0x00) // Preamble check
    {
        return DECODE_FAIL_MIC; // Wrong preamble or noise
    }

    // Signal repeats a transmission 8x so we check for 8 matches excluding the preamble, reduces error.
    // If 4 of the 8 don't match then reject
    // If (b[3] != b[6] || b[9] != b[12] || b[15] != b[18] || b[21] != b[24])
    // If none of the repeating signals match we know its 100% an error
    for (uint8_t i = 3; i < 24; i += 3) {
        if (b[i + filter_count] != b[i + 3 + filter_count]) // 3 6, 9 12, 15 18, 21 24
        {
            filter_error++;
            if (filter_error > 3) // 4
            {
                return DECODE_FAIL_MIC; // If more than 4 don't match. Can be fine tuned
            }
        }
        filter_count + 3; // 3 6, 9 12, 15 18, 21 24
    }

    // ffffff32bd0832bd0832bd0832bd0832bd0832bd0832bd0832bd08
    // 000000cd42f7cd42f7cd42f7cd42f7cd42f7cd42f7cd42f7cd42f7 when inverted
    // 000000 cd42f7 cd42f7 cd42f7 cd42f7 cd42f7 cd42f7 cd42f7 cd42f7
    // 110110100100 = 1100
