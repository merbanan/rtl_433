/** @file
    ...
    Copyright (C) 2022 Andre Lohmann <lohmann.andre@gmail.com>

    With the inspiration of Haras1111, jurs, DrDiettrich, agmue and FriedChips from the following thread:
    https://forum.arduino.cc/t/funksignal-decodieren-fur-masterarbeit-fishfinder-zweckentfremden/439091
...
!!! FUnctions are copied from vaillant_vrt340f.c !!!
...
static int validate_checksum(r_device *decoder, uint8_t *b, int from, int to, int cs_from, int cs_to)
{
    ...
    if (chk) {
        if (decoder->verbose) {
            fprintf(stderr, "Checksum error in LUCKY Wireless Transducer.  Expected: %04x  Calculated: %04x\n", expected, calculated);
...
r_device lucky_transducer = {
        .name        = "LUCKY Wireless Transducer - Lucky Fishfinder FF718LiC-W, FF1108-1CW, FFW718 Transducer",
