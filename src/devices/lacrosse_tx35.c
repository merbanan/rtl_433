...
static int lacrosse_it(r_device *decoder, bitbuffer_t *bitbuffer, int device29or35)
{
    ...
    for (int row = 0; row < bitbuffer->num_rows; ++row) {
        ...
        if (decoder->verbose)
            decoder_logf(decoder, 0, __func__, "LaCrosse TX29/35 detected, buffer is %d bits length, device is TX%d", bitbuffer->bits_per_row[row], device29or35);
        ...
        if (r_crc != c_crc) {
            if (decoder->verbose)
                decoder_logf(decoder, 0, __func__, "LaCrosse TX29/35 bad CRC: calculated %02x, received %02x", c_crc, r_crc);
            // reject row
