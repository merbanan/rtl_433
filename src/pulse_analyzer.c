...
void pulse_analyzer(pulse_data_t *data, int package_type) {
        ...
        histogram_sum(&hist_gaps, data->gap, data->num_pulses - 1,
                      TOLERANCE);                      // Leave out last gap (end)
        histogram_sum(&hist_periods, pulse_periods.pulse, pulse_periods.num_pulses - 1,
                      TOLERANCE); // Leave out last gap (end)
        ...
        if (data->num_pulses == 1) {
                ...
                device.gap_limit =
                        to_us * (hist_gaps.bins[1].max + 1);                        // Set limit above next lower gap
                device.reset_limit =
                        to_us * (hist_gaps.bins[hist_gaps.bins_count - 1].max + 1); // Set limit above biggest gap
                ...
                device.modulation = (package_type == PULSE_DATA_FSK) ? FSK_PULSE_MANCHESTER_ZEROBIT
                                                                     : OOK_PULSE_MANCHESTER_ZEROBIT;
                device.short_width = to_us * MIN(hist_pulses.bins[0].mean,
                                                 hist_pulses.bins[1].mean); // Assume shortest pulse is half period
                ...
                device.reset_limit =
                   ...
                   && (abs(hist_pulses.bins[1].mean - 2 * hist_pulses.bins[0].mean) <=
                       hist_pulses.bins[0].mean / 8)    // Pulses are multiples of shortest pulse
                   && (abs(hist_pulses.bins[2].mean - 3 * hist_pulses.bins[0].mean) <= hist_pulses.bins[0].mean / 8)
                   && (abs(hist_gaps.bins[0].mean - hist_pulses.bins[0].mean) <=
                       hist_pulses.bins[0].mean / 8)    // Gaps are multiples of shortest pulse
                ...
                device.modulation = (package_type == PULSE_DATA_FSK) ? FSK_PULSE_PWM : OOK_PULSE_PWM;
                device.short_width =
                        to_us * (p1 < p2 ? p1 : p2);                                // Set to shorter pulse width
                device.long_width =
                        to_us * (p1 < p2 ? p2 : p1);                                // Set to longer pulse width
                device.sync_width =
                        to_us * hist_pulses.bins[0].mean;                           // Set to lowest count pulse width
                device.reset_limit =
                        to_us * (hist_gaps.bins[hist_gaps.bins_count - 1].max + 1); // Set limit above biggest gap
        } else {
        ...
        if (device.modulation) {
                fprintf(stderr,
                        "Attempting demodulation... short_width: %.0f, long_width: %.0f, reset_limit: %.0f, sync_width: %.0f\n",
                ...
                switch (device.modulation) {
                        ...
                        case OOK_PULSE_PPM:
                                fprintf(stderr,
                                        "Use a flex decoder with -X 'n=name,m=OOK_PPM,s=%.0f,l=%.0f,g=%.0f,r=%.0f'\n",
                                        device.short_width, device.long_width,
                                        device.gap_limit, device.reset_limit);
                                data->gap[data->num_pulses - 1] =
                                        device.reset_limit / to_us + 1; // Be sure to terminate package
                                pulse_slicer_ppm(data, &device);
                                break;
                        case OOK_PULSE_PWM:
                                fprintf(stderr,
                                        "Use a flex decoder with -X 'n=name,m=OOK_PWM,s=%.0f,l=%.0f,r=%.0f,g=%.0f,t=%.0f,y=%.0f'\n",
                                        device.short_width, device.long_width, device.reset_limit,
                        ...
                        case FSK_PULSE_PWM:
                                fprintf(stderr,
                                        "Use a flex decoder with -X 'n=name,m=FSK_PWM,s=%.0f,l=%.0f,r=%.0f,g=%.0f,t=%.0f,y=%.0f'\n",
                                        device.short_width, device.long_width, device.reset_limit,
                                        device.gap_limit, device.tolerance, device.sync_width);
                                data->gap[data->num_pulses - 1] =
                                        device.reset_limit / to_us + 1; // Be sure to terminate package
                        ...
                        case OOK_PULSE_MANCHESTER_ZEROBIT:
                                fprintf(stderr,
                                        "Use a flex decoder with -X 'n=name,m=OOK_MC_ZEROBIT,s=%.0f,l=%.0f,r=%.0f'\n",
                                        device.short_width, device.long_width, device.reset_limit);
                                data->gap[data->num_pulses - 1] =
                                        device.reset_limit / to_us + 1; // Be sure to terminate package
