...
This decoder handles three frame types -- temperature-update, periodic-update,
and status-reply -- all sharing the same leading payload layout (device ID,
shared ID material, battery, temperature), shifted by one byte between
temperature-update and the other two:
...
change, "long battery life" toggle) on both physical units. Not decoded:
unlike the status-reply collision above, ping frames carry no battery or
temperature data to cross-check, and the 0x70 marker / LL=0x1c combination is
shared with other Govee gateway devices (confirmed on the H5112 dual-probe
thermometer), making device attribution by frame structure alone impossible.
These frames are silently ignored (DECODE_ABORT_EARLY) rather than risk
misattributing another device's ping to this one.
...
this decoder and are ignored. The H5310 decoder runs at the default priority
(before H5059's reduced priority, see govee_h5059.c) so it gets first access
to frames in this family.
*/
