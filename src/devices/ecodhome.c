...
static int ecodhome_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (bitbuffer->num_rows != 1 || bitbuffer->bits_per_row[0] < 128) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "to few bits (%u)", bitbuffer->bits_per_row[0]);
    ...
    if (start_pos + 12 * 8 >= bitbuffer->bits_per_row[0]) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "message too short (%u)", bitbuffer->bits_per_row[0] - start_pos);
        return DECODE_ABORT_LENGTH; // message too short
    }

    bitbuffer_extract_bytes(bitbuffer, 0, start_pos, msg, sizeof(msg) * 8);
    if (decoder->verbose > 1) {
        decoder_log_bitrow(decoder, 0, __func__, msg, sizeof(msg) * 8, "MSG");
    }
    ...
    if (m_type == 0x7700) {
        ...
        if ((sum & 0xff) != msg[11]) {
            if (decoder->verbose > 1)
                decoder_logf(decoder, 0, __func__, "checksum fail %02x vs %02x", sum, msg[9]);
            return DECODE_FAIL_MIC;
        }
        if (msg[10] != 0x53) {
            if (decoder->verbose > 1)
                decoder_logf(decoder, 0, __func__, "wrong stop byte %02x", msg[10]);
        ...
        if (msg[11] != 0x00) {
            if (decoder->verbose > 1)
                decoder_logf(decoder, 0, __func__, "wrong poststop byte %02x", msg[11]);
            return DECODE_FAIL_SANITY;
        }
