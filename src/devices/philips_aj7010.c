...
static int philips_aj7010_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (xor_bytes(b, 5) && xor_bytes(b, 3) ^ b[4]) {
        if (decoder->verbose) {
            decoder_log(decoder, 0, __func__, "bad checksum");
        }
    ...
    if (decoder->verbose) {
        decoder_logf(decoder, 0, __func__, "channel decoded is %d", channel);
    }
    ...
    if (decoder->verbose) {
        decoder_logf(decoder, 0, __func__, "temperature: raw: %d %08X converted: %.2f", temp_raw, temp_raw, temp_c);
    }
