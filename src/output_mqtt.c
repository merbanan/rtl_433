...
mqtt_client_init(struct mg_mgr *mgr, tls_opts_t *tls_opts, char const *host, char const *port, char const *user,
                 char const *pass, char const *client_id, int retain, int qos) {
...
struct data_output *data_output_mqtt_create(struct mg_mgr *mgr, char *param, char const *dev_hint) {
        ...
        while (getkwargs(&opts, &key, &val)) {
                ...
                if (!key || !*key)
                        ...
                        fprintf(stderr,
                                "for \"afterid\"   use e.g. \"devices=rtl_433/[hostname]/devices[/type][/model][/subtype][/id][/channel]\"\n");
                        fprintf(stderr,
                                "for \"beforeid\"  use e.g. \"devices=rtl_433/[hostname]/devices[/type][/model][/subtype][/channel][/id]\"\n");
                        fprintf(stderr,
                                "for \"replaceid\" use e.g. \"devices=rtl_433/[hostname]/devices[/type][/model][/subtype][/channel]\"\n");
                        fprintf(stderr,
                                "for \"no\"        use e.g. \"devices=rtl_433/[hostname]/devices[/type][/model][/subtype][/id]\"\n");
                        exit(1);
                }
