...
char const *file_info_string(file_info_t *info) {
        switch (info->format) {
                case CU8_IQ:
                        return "CU8 IQ (2ch uint8)";
                case S16_AM:
                        return "S16 AM (1ch int16)";
                case S16_FM:
                        return "S16 FM (1ch int16)";
                case CF32_IQ:
                        return "CF32 IQ (2ch float32)";
                case CS16_IQ:
                        return "CS16 IQ (2ch int16)";
                case F32_AM:
                        return "F32 AM (1ch float32)";
                case F32_FM:
                        return "F32 FM (1ch float32)";
                case F32_I:
                        return "F32 I (1ch float32)";
                case F32_Q:
                        return "F32 Q (1ch float32)";
                case VCD_LOGIC:
                        return "VCD logic (text)";
                case U8_LOGIC:
                        return "U8 logic (1ch uint8)";
                case PULSE_OOK:
                        return "OOK pulse data (text)";
                default:
                        return "Unknown";
        }
}
        ...
        while (*p) {
                if (*p >= '0' && *p <= '9') {
                        ...
                        if (len == 1 && !strncasecmp("M", s, 1)) info->center_frequency = num * 1e6;
                                ...
                                info->center_frequency = num * scale;
                        else if (len == 4 && !strncasecmp("sps", s + 1, 3) && scale > 1.0)
                                info->sample_rate = num * scale;
                        ...
                        else if (len == 4 && !strncasecmp("data", t, 4))
                                file_type_set_format(&info->format, F_CU8); // compat
                        ...
                        else if (len == 5 && !strncasecmp("cfile", t, 5))
                                file_type_set_format(&info->format, F_CF32); // compat
                        else if (len == 5 && !strncasecmp("logic", t, 5)) file_type_set_content(&info->format, F_LOGIC);
                        else if (len == 3 && !strncasecmp("complex16u", t, 10))
                                file_type_set_format(&info->format, F_CU8); // compat
                        else if (len == 3 && !strncasecmp("complex16s", t, 10))
                                file_type_set_format(&info->format, F_CS8); // compat
                        else if (len == 4 && !strncasecmp("complex", t, 7))
                                file_type_set_format(&info->format, F_CF32); // compat
