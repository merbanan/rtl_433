/*
 * Sensible Living LLC Mini-Plant Moisture Sensor
 *
 * Note these sensors use Mike McCauley's VirtualWire protocol
 * which is virtually identical to Mike McCauley's RadioHead protocol.
 *
 * This source is essentially a copy of radiohead_ask.c with some changes.
 */
...
static uint8_t payload[SL_ASK_MAX_PAYLOAD_LEN] = {0};
...
static int sensible_living_callback(bitbuffer_t *bitbuffer) {
    ...
    house_id = payload[1];
    module_id = payload[2] * 256 + payload[3];
    sensor_type = payload[4];
    sensor_count = payload[5];
    alarms = payload[6];
    sensor_value = payload[7] * 256 + payload[8];
    battery_voltage = payload[9] * 256 + payload[10];
