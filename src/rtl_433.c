...
static void sdr_callback(unsigned char *iq_buf, uint32_t len, void *ctx) {
        ...
        int process_frame =
                demod->squelch_offset <= 0 || !noise_only || demod->load_info.format || demod->analyze_pulses ||
        ...
        if (noise_only) {
                ...
                if (demod->auto_level > 0 && demod->noise_level < demod->min_level - 3.0f
                    && fabsf(demod->min_level_auto - demod->noise_level - 3.0f) > 1.0f) {
                        ...
                        fprintf(stderr,
                                "Estimated noise level is %.1f dB, adjusting minimum detection level to %.1f dB\n",
                                demod->noise_level, demod->min_level_auto);
                        pulse_detect_set_levels(demod->pulse_detect, demod->use_mag_est, demod->level_limit,
                                                demod->min_level_auto, demod->min_snr, demod->detect_verbosity);
                }
        } else {
        ...
        if (demod->enable_FM_demod && process_frame) {
                ...
                if (demod->sample_size == 2) { // CU8
                        baseband_demod_FM(iq_buf, demod->buf.fm, n_samples, cfg->samp_rate, low_pass,
                                          &demod->demod_FM_state);
                } else { // CS16
                        baseband_demod_FM_cs16((int16_t *) iq_buf, demod->buf.fm, n_samples, cfg->samp_rate, low_pass,
                                               &demod->demod_FM_state);
                }
        }
        ...
        if (demod->r_devs.len || demod->analyze_pulses || demod->dumper.len || demod->samp_grab) {
                ...
                while (package_type && process_frame) {
                        ...
                        if (package_type == PULSE_DATA_OOK) {
                                ...
                                for (void **iter = demod->dumper.elems; iter && *iter; ++iter) {
                                        ...
                                        if (dumper->format == U8_LOGIC)
                                                pulse_data_dump_raw(demod->u8_buf, n_samples, cfg->input_pos,
                                                                    &demod->pulse_data, 0x02);
                        ...
                        if (demod->samp_grab) {
                                if (cfg->grab_mode == 1
                                    ...
                                    || (cfg->grab_mode == 3 && demod->frame_event_count > 0)) {
                                        unsigned frame_pad =
                                                n_samples / 8; // this could also be a fixed value, e.g. 10000 samples
                ...
                for (void **iter = demod->dumper.elems; iter && *iter; ++iter) {
                        ...
                        if (dumper->format == U8_LOGIC) {
                                ...
                                pulse_data_dump_raw(demod->u8_buf, n_samples, cfg->input_pos, &demod->fsk_pulse_data,
                                                    0x04);
                                break;
                        }
                }
                ...
                if (dumper->format == CU8_IQ) {
                        if (demod->sample_size == 4) {
                                for (unsigned long n = 0; n < n_samples * 2; ++n)
                                        ((uint8_t *) demod->buf.temp)[n] =
                                                (((int16_t *) iq_buf)[n] / 256) + 128; // scale Q0.15 to Q0.7
                        ...
                        if (demod->sample_size == 2) {
                                for (unsigned long n = 0; n < n_samples * 2; ++n)
                                        ((int16_t *) demod->buf.temp)[n] =
                                                (iq_buf[n] * 256) - 32768; // scale Q0.7 to Q0.15
                        ...
                        if (demod->sample_size == 2)
                                ...
                                for (unsigned long n = 0; n < n_samples; ++n)
                                        demod->f32_buf[n] =
                                                ((int16_t *) iq_buf)[n * 2] * (1.0f / 0x8000); // scale from Q0.15
                        out_buf = (uint8_t *) demod->f32_buf;
                        out_len = n_samples * sizeof(float);
                        ...
                        if (demod->sample_size == 2)
                                for (unsigned long n = 0; n < n_samples; ++n)
                                        demod->f32_buf[n] =
                                                (iq_buf[n * 2 + 1] - 128) * (1.0f / 0x80); // scale from Q0.7
                        else
                                for (unsigned long n = 0; n < n_samples; ++n)
                                        demod->f32_buf[n] =
                                                ((int16_t *) iq_buf)[n * 2 + 1] * (1.0f / 0x8000); // scale from Q0.15
                        out_buf = (uint8_t *) demod->f32_buf;
                        out_len = n_samples * sizeof(float);
        ...
        switch (opt) {
                ...
                case 'M':
                        ...
                        if (!strncasecmp(arg, "time", 4)) {
                                ...
                                while (p && *p) {
                                        if (!strncasecmp(p, "0", 1) || !strncasecmp(p, "no", 2) ||
                                            !strncasecmp(p, "off", 3))
                                                cfg->report_time = REPORT_TIME_OFF;
                                        else if (!strncasecmp(p, "1", 1) || !strncasecmp(p, "yes", 3) ||
                                                 !strncasecmp(p, "on", 2))
                                                cfg->report_time = REPORT_TIME_DATE;
                ...
                case 'R':
                        ...
                        if ((n > 0 && cfg->devices[n - 1].disabled > 2) ||
                            (n < 0 && cfg->devices[-n - 1].disabled > 2)) {
                ...
                case 'Y':
                        ...
                        while (p && *p) {
                                ...
                                if (kwargs_match(p, "autolevel", &val))
                                        cfg->demod->auto_level = atoiv(val,
                                                                       1); // arg_float_default(p + 9, "-Y autolevel: ");
                                else if (kwargs_match(p, "squelch", &val))
                                        cfg->demod->squelch_offset = atoiv(val,
                                                                           1); // arg_float_default(p + 7, "-Y squelch: ");
...
static void sdr_handler(sdr_event_t *ev, void *ctx) {
        ...
        if (ev->ev & SDR_EV_FREQ) {
                ...
                data = data_append(data,
                                   ...
                                   "frequencies", "", DATA_COND, cfg->frequencies > 1, DATA_ARRAY,
                                   data_array(cfg->frequencies, DATA_INT, cfg->frequency),
                                   "hop_times", "", DATA_COND, cfg->frequencies > 1, DATA_ARRAY,
...
int main(int argc, char **argv) {
        ...
        int dbus_status = 0;
        dbus_status = init_dbus_connection();
        if (dbus_status != 0) {
                fprintf(stderr, "dbus-connection error: could not connect");
                exit(1);
        }
        ...
        pulse_detect_set_levels(demod->pulse_detect, demod->use_mag_est, demod->level_limit, demod->min_level,
        ...
        if (cfg->in_files.len) {
                ...
                for (void **iter = cfg->in_files.elems; iter && *iter; ++iter) {
                        ...
                        cfg->center_frequency = demod->load_info.center_frequency ? demod->load_info.center_frequency
                                                                                  : cfg->frequency[0];
                        ...
                        do {
                                ...
                                if (cfg->in_replay) {
                                        // per block delay
                                        unsigned delay_us = (unsigned) (1000000llu * DEFAULT_BUF_LENGTH /
                                                                        cfg->samp_rate / demod->sample_size /
                                                                        cfg->in_replay);
                                ...
                                if (demod->load_info.format == CF32_IQ) {
                                        n_read = fread(test_mode_float_buf, sizeof(float), DEFAULT_BUF_LENGTH / 2,
                                                       in_file);
                                ...
                                demod->sample_file_pos =
                                        ((float) n_blocks * DEFAULT_BUF_LENGTH + n_read) / cfg->samp_rate /
                                        demod->sample_size;
                        ...
                        demod->sample_file_pos =
                                ((float) n_blocks + 1) * DEFAULT_BUF_LENGTH / cfg->samp_rate / demod->sample_size;
