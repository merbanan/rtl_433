...
#define ID_RTGR328N  0x0cc3 // warning: id is from 0x0cc3 and 0xfcc3, same as RTGN318 but different packet size
#define ID_PCR800   0x2914 
#define ID_THGR810	0xf824
#define ID_WGR800	0x1984
#define ID_CM160	0x5001
...
static int oregon_scientific_v2_1_parser(bitbuffer_t *bitbuffer) {
  ...
  if( ((bb[0][1] == 0x55) && (bb[0][2] == 0x55)) ||
    ...
    if ((sensor_id == ID_THGR122N) || (sensor_id == ID_THGR968)){
      ...
      fprintf(stdout,"no jestem tukej %d %d\n",sensor_id, num_valid_v2_bits);
      if (num_valid_v2_bits==153 && (validate_os_v2_message(msg, 153, num_valid_v2_bits, 15) == 0)) {
        ...
        data = data_make("time",         "",            DATA_STRING, time_str,
            ...
            NULL);
        data_acquired_handler(data);
      } else if (num_valid_v2_bits==223 && (validate_os_v2_message(msg, 223, num_valid_v2_bits, 15) == 0)) {
        data = data_make("time",         "",            DATA_STRING, time_str,
            "model",         "",            DATA_STRING, "Thermo Hygro RF Clock Sensor THGR328N/RTGR328N",
            "id",            "House Code",  DATA_INT,    get_os_rollingcode(msg, sensor_id),
            "channel",       "Channel",     DATA_INT,    get_os_channel(msg, sensor_id), // 1 to 5
            "battery",       "Battery",     DATA_STRING, get_os_battery(msg, sensor_id) ? "LOW" : "OK",
            "temperature_C", "Temperature", DATA_FORMAT, "%.02f C", DATA_DOUBLE, get_os_temperature(msg, sensor_id),
...
*/
	...
	data_acquired_handler(data);
      ...
      return 1;
       ...
       int battery = (msg[2] >> 2 & 0x01);
       float rawAmp = (msg[4] >> 4 << 8 | (msg[3] & 0x0f )<< 4 | msg[3] >> 4);
       float Amp=rawAmp*3.5/100;
       float Watt=Amp*230; //This is assumption, that Voltage is 230V
       int rolling_code = ((msg[1] << 4)&0xF0) | ((msg[2] >> 4)&0x0F);
       data = data_make("time",  "",   DATA_STRING,  time_str,
         "model",    "",   DATA_STRING,  "Energy Sensor CM160 Current",
         "id",     "House Code", DATA_INT, rolling_code,
         "battery",    "Battery",  DATA_STRING,  battery?"LOW":"OK",
         "current",    "Current",  DATA_FORMAT, "%.3f A",DATA_DOUBLE, Amp,
         "power",      "Power", DATA_FORMAT, "%.0f W", DATA_DOUBLE, Watt,  
         NULL);
       data_acquired_handler(data);
    } else if ((msg[0] == 0x20) || (msg[0] == 0x21) || (msg[0] == 0x22)
        || (msg[0] == 0x23) || (msg[0] == 0x24)) { //  Owl CM160 Readings (TB: I think, that here should be CM180 instead of CM160)
      msg[0]=msg[0] & 0x0f;
