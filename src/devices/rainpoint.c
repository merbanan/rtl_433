...
static int rainpoint_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (len - start_pos != 12 * 2 * 8) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "Manchester decode failed, got %u bits", len - start_pos);
        return DECODE_ABORT_LENGTH;
    }
    ...
    reflect_bytes(b, 12);
    if (decoder->verbose > 1) {
        decoder_log_bitrow(decoder, 0, __func__, b, 12 * 8, "");
    }
    ...
    if ((sum & 0xff) != b[10]) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "Checksum failed %04x vs %04x", b[10], sum);
