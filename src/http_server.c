...
static void ev_handler(struct mg_connection *nc, int ev, void *ev_data) {
        switch (ev) {
                ...
                case MG_EV_WEBSOCKET_HANDSHAKE_DONE: {
                        ...
                        data_free(meta);
                        /* Send history */
                        for (void **iter = ring_list_iter(ctx->history); iter; iter = ring_list_next(ctx->history,
                                                                                                     iter))
...
static struct http_server_context *
http_server_start(struct mg_mgr *mgr, char const *host, char const *port, r_cfg_t *cfg, struct data_output *output) {
