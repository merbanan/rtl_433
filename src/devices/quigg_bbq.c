...
static int quigg_bbq_callback(r_device *decoder, bitbuffer_t *bitbuffer) {
    bitrow_t *bb = bitbuffer->bb;
    data_t *data;

    if (decoder->verbose > 1) {
        fprintf(stderr,"Possible Quigg BBQ: ");
    ...
    uint16_t tempf = ((data_bytes[2]>>6) | data_bytes[0]) -90;

    data = data_make(
                "model",         "",            DATA_STRING, _X("QUIGG","GT-TMBBQ-05"),
