...
static int fineoffset_wh31l_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (decoder->verbose)
        decoder_logf(decoder, 0, __func__, "WH31L detected, buffer is %d bits length", bitbuffer->bits_per_row[row]);
    ...
    if (c_crc) {
        if (decoder->verbose)
            decoder_log(decoder, 0, __func__, "bad CRC");
        return DECODE_FAIL_MIC;
    }
    uint8_t c_sum = add_bytes(b, 8) - b[8];
    if (c_sum) {
        if (decoder->verbose)
            decoder_log(decoder, 0, __func__, "bad SUM");
        return DECODE_FAIL_MIC;
    }
