...
static int oregon_scientific_sl109h_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    for (int row_index = 0; row_index < bitbuffer->num_rows; row_index++) {
        ...
        if (sum != chk) {
            if (decoder->verbose > 1)
                decoder_logf_bitbuffer(decoder, 0, __func__, bitbuffer, "Checksum error. Expected: %01x Calculated: %01x", chk, sum);
        ...
        if (temp_c < -20 || temp_c > 60) {
            if (decoder->verbose > 1)
                decoder_logf(decoder, 0, __func__, "temperature sanity check failed: %.1f C", temp_c);
