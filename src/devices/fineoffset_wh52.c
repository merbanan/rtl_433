...
- Byte 20: battery voltage; battery_mV ~= b20 * 20 (0.02 V/LSB, no offset; empirical. Reported as battery_ok_b (low below 1.4 V) + battery_pct + battery_mV.
...
static int fineoffset_wh52_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    // We have no data yet for the battery depletion curve / remaining percentage.
    // Assuming similarity to the WH51 sensor and reusing its mapping for a start.
    // Review and correct when more ground data is available.
    float battery_level;
    if (battery_mv >= 1600) {
      battery_level = 1.0;
    } else if (battery_mv >= 1500) {
      battery_level = 0.9;
    } else if (battery_mv >= 1400) {
      battery_level = 0.5;
