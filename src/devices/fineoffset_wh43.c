...
NOTE: this is mostly just copied from WH0290 and needs to be updated with info form #3179 !
...
static int fineoffset_wh43_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (crc != b[8] || sum != b[9]) {
        decoder_logf(decoder, 1, __func__, "Checksum error");
    ...
    int battery_bars = ((b[4] & 0x40) >> 4) | ((b[6] & 0xC0) >> 6);
