...
- Checksum         {16} [CRC-8](~[CRC-8] for a probe, 00 for a booster)
...
static int thermopro_tp86xb_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    int payload_len = msg_len - offset;
    if (payload_len < 72) {
        decoder_logf(decoder, 1, __func__, "Payload too short: %d bits", payload_len);
        return DECODE_ABORT_LENGTH;
    }

    bitbuffer_extract_bytes(bitbuffer, 0, offset, b, 9 * 8);

    // Validate Checksum format. Bytes 7 and 8 must not share common 1-bits.
    if ((b[7] & b[8]) != 0) {
        decoder_logf(decoder, 2, __func__, "Bytes 7 and 8 must not overlap. Actual: (%02x & %02x) = %02x", b[7], b[8], b[7] & b[8]);
        return DECODE_FAIL_MIC;
    }

    // Validate Checksum A: CRC-8 (Poly 0x07, Init 0x00, Final XOR 0xDB).
    uint8_t calc_crc = crc8(b, 7, 0x07, 0x00) ^ 0xdb;
    if (calc_crc != b[7]) {
        decoder_logf(decoder, 2, __func__, "Integrity check failed %02x vs %02x (calculated)", b[7], calc_crc);
        return DECODE_FAIL_MIC;
    }
