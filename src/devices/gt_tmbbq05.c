...
static int gt_tmbbq05_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (!b[0] && !b[1] && !b[2] && !b[3]) {
        if (decoder->verbose > 1) {
            decoder_log(decoder, 0, __func__, "DECODE_FAIL_SANITY data all zero");
        }
    ...
    if (parity_bytes(p, 4)) {
        if (decoder->verbose > 1) {
            decoder_log(decoder, 0, __func__, "gt_tmbbq05_decode: parity check failed (should be ODD)");
        }
    ...
    if ((sum & 0xf) != (b[3] & 0xf)) {
        if (decoder->verbose > 1) {
            decoder_logf_bitrow(decoder, 0, __func__, b, 32, "Bad checksum (%x)", sum);
        }
        return DECODE_FAIL_MIC;
    }
