...
static int lacrosse_ws7000_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (type > 5) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "LaCrosse-WS7000: unhandled sensor type (%d)", type);
        return DECODE_ABORT_EARLY;
    }
    ...
    if (len < data_len) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "LaCrosse-WS7000: short data (%u of %u)", len, data_len);
        return DECODE_ABORT_LENGTH;
    }
    ...
    if (xor_bytes(b, len - 1)) {
        if (decoder->verbose > 1)
            decoder_log(decoder, 0, __func__, "LaCrosse-WS7000: checksum error (xor)");
    ...
    if (((add_bytes(b, len - 1) + 5) & 0xf) != b[len - 1]) {
        if (decoder->verbose > 1)
            decoder_log(decoder, 0, __func__, "LaCrosse-WS7000: checksum error (add)");
        return DECODE_FAIL_MIC;
    }
