...
void baseband_low_pass_filter(uint16_t const *x_buf, int16_t *y_buf, uint32_t len, filter_state_t *state) {
        ...
        y_buf[0] =
                (a[1] * state->y[0] + b[0] * (x_buf[0] + state->x[0])) >> (F_SCALE - 1); // note: prescaled, b[0]==b[1]
        for (unsigned long i = 1; i < len; i++) {
                y_buf[i] = (a[1] * y_buf[i - 1] + b[0] * (x_buf[i] + x_buf[i - 1]))
                        >> (F_SCALE - 1); // note: prescaled, b[0]==b[1]
        }
...
void
baseband_demod_FM(uint8_t const *x_buf, int16_t *y_buf, unsigned long num_samples, uint32_t samp_rate, float low_pass,
...
void baseband_demod_FM_cs16(int16_t const *x_buf, int16_t *y_buf, unsigned long num_samples, uint32_t samp_rate,
                            float low_pass, demodfm_state_t *state) {
        // Select filter coeffs, [b,a] = butter(1, cutoff)
        // e.g [b,a] = butter(1, 0.1) -> 3x tau (95%) ~10 samples, 250k -> 40us, 1024k -> 10us
        // a = 1.00000, 0.72654; b = 0.13673, 0.13673;
        // e.g. [b,a] = butter(1, 0.2) -> 3x tau (95%) ~5 samples, 250k -> 20us, 1024k -> 5us
        ...
        for (unsigned n = 0; n < num_samples; n++) {
                ...
                // Calculate phase difference vector: x[n] * conj(x[n-1])
                pr = (int64_t) x0r * x1r +
                     (int64_t) x0i * x1i; // May exactly overflow an int32_t (-32768*-32768 + -32768*-32768)
