...
static int ambientweather_whx_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    for (row = 0; row < bitbuffer->num_rows; ++row) {
        ...
        if (msg_type == wh31e_type_code || msg_type == wh31b_type_code) {
            ...
            int rain_raw    = (b[5] << 8) | b[6];
            char extra[11];
            // b[7] is unknown, b[8]/b[9] are CRC/SUM; b[10..14] are past WN20's own
            // 10-byte payload, always present due to the shared 18-byte extraction buffer
            ...
            data_t *data = data_make(
                    ...
                    "battery_ok",       "Battery OK",      DATA_INT,    battery_lvl > 0,
                    "battery_pct",      "Battery level",   DATA_INT,    battery_lvl, // Note: this might change with #3103
                    "rain_mm",          "Total Rain",      DATA_FORMAT, "%.1f mm", DATA_DOUBLE, rain_raw * 0.1,
                    "data",             "Extra Data",      DATA_STRING, extra,
                    "mic",              "Integrity",       DATA_STRING, "CRC",
                    NULL);
