...
/* Bitwise implementation of CRC-8 with parameters:
 * width=8  poly=0x31  init=0x6c  refin=false  refout=false  xorout=0x00
 */
uint8_t crc8_rubicson(uint8_t const *data, size_t len)
{
    unsigned crc = 0x6c;
    while (len--) {
        crc ^= *data++;
...
static int rubicson_crc_check(bitrow_t *bb) {
    uint8_t tmp[5];
