...
static int ikea_sparsnas_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if ((bitbuffer->bits_per_row[0] < IKEA_SPARSNAS_MESSAGE_BITLEN) || (bitbuffer->bits_per_row[0] > IKEA_SPARSNAS_MESSAGE_BITLEN_MAX)) {
        if (decoder->verbose > 1) {
            ...
            decoder_logf(decoder, 0, __func__, "Too short or too long packet received. Expected %d, received %d", IKEA_SPARSNAS_MESSAGE_BITLEN, bitbuffer->bits_per_row[0]);
        }
    ...
    if ((bitbuffer->bits_per_row[0] == bitpos) || (bitpos + IKEA_SPARSNAS_MESSAGE_BITLEN > bitbuffer->bits_per_row[0])) {
        if (decoder->verbose > 1) {
            decoder_output_bitbufferf(decoder, bitbuffer, "%s: ", __func__);
            decoder_log(decoder, 0, __func__, "malformed package, preamble not found. (Expected 0xAAAAD201)");
        }
    ...
    if (!ikea_sparsnas_sensor_id) {
        if (decoder->verbose > 1) {
            decoder_log(decoder, 0, __func__, "No sensor ID configured. Brute forcing encryption.");
        }
        ikea_sparsnas_sensor_id = ikea_sparsnas_brute_force_encryption(buffer);
        if (decoder->verbose > 1) {
            if (ikea_sparsnas_sensor_id) {
                decoder_logf(decoder, 0, __func__, "Found valid sensor ID %06u. If reported values does not make sense, this might be incorrect.", ikea_sparsnas_sensor_id);
            } else {
                decoder_log(decoder, 0, __func__, "No valid sensor ID found.");
            }
        }
    }
    ...
    if (decoder->verbose > 1) {
        decoder_logf(decoder, 0, __func__, "CRC OK (%X == %X)", crc_calculated, crc_received);
        decoder_logf(decoder, 0, __func__, "Encryption key: 0x%X%X%X%X%X", key[0], key[1], key[2], key[3], key[4]);
        decoder_output_bitrowf(decoder, decrypted, 18 * 8, "Decrypted");
        decoder_logf(decoder, 0, __func__, "Received sensor id: %06u", rcv_sensor_id);
    }

    if (rcv_sensor_id != ikea_sparsnas_sensor_id) {
        if (decoder->verbose > 1) {
            decoder_logf(decoder, 0, __func__, "Malformed package, or wrong sensor id. Received sensor id (%06u) not the same as sender (%d)", rcv_sensor_id, ikea_sparsnas_sensor_id);
        }
    }
    ...
    if (decrypted[0] != 0x11) {
        ...
        if (decoder->verbose > 1) {
            decoder_logf(decoder, 0, __func__, "Message malformed (byte0=%X expected %X)", decrypted[0], 0x11);
        }
    ...
    if (decrypted[3] != 0x07) {
        decoder_output_bitrowf(decoder, decrypted + 5, 13 * 8,  "Message malformed");
        if (decoder->verbose > 1) {
            decoder_logf(decoder, 0, __func__, "Message malformed (byte3=%X expected %X)", decrypted[0], 0x07);
        }
        return DECODE_FAIL_SANITY;
    }
