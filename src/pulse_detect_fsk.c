        ...
        switch (s->fsk_state) {
                ...
                case PD_FSK_STATE_FH:        // Pulse high at F1 frequency
                        ...
                        if (fm_f1_delta > fm_f2_delta) {
                                ...
                                if (s->fsk_pulse_length >= PD_MIN_PULSE_SAMPLES) {
                                        ...
                                        s->fsk_pulse_length += fsk_pulses->gap[fsk_pulses->num_pulses -
                                                                               1];    // Restore counter
                                ...
                                if (fm_n > s->fm_f1_est)
                                        s->fm_f1_est +=
                                                fm_n / FSK_EST_FAST - s->fm_f1_est / FSK_EST_FAST;    // Fast estimator
                                else
                                        s->fm_f1_est +=
                                                fm_n / FSK_EST_SLOW - s->fm_f1_est / FSK_EST_SLOW;    // Slow estimator
                        }
                        break;
                case PD_FSK_STATE_FL:        // Pulse gap at F2 frequency
                        ...
                        if (fm_f2_delta > fm_f1_delta) {
                                ...
                                if (fm_n < s->fm_f2_est)
                                        s->fm_f2_est +=
                                                fm_n / FSK_EST_FAST - s->fm_f2_est / FSK_EST_FAST;    // Fast estimator
                                else
                                        s->fm_f2_est +=
                                                fm_n / FSK_EST_SLOW - s->fm_f2_est / FSK_EST_SLOW;    // Slow estimator
                        }
                        break;
