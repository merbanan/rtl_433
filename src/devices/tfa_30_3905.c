/** @file
    ...
    TFA Dostmann 30.3902 T pro sensor with external temp sensor.

    Copyright (c) 2025 Jacob Maxa <jack77@gmx.net>
...
/*
    ...
    All packets starting with a 0xaaaaaaaa SYNC followed by a 0x4b2dd42b24 preamble
    ...
    All packets starting with a 0xaaaaaaaa SYNC followed by a 0x4b2dd42b1e preamble
...
#define MESSAGE_BITLEN TFA_303905_MESSAGE_BITLEN


uint32_t crc32_reveng(const uint8_t *data, size_t len, uint32_t init);
...
/* *
 * Bitwise freflection of a double word for the crc32 calculation
 * */
...
uint32_t crc32_reveng(const uint8_t *data, size_t len, uint32_t init) {
    ...
    uint32_t crc = init;   // init wie in RevEng

    for (size_t i = 0; i < len; i++) {
        uint8_t b = data[i];

        // refin=true ⇒ Eingabebyte spiegeln
        b = reflect8(b);

        crc ^= ((uint32_t)b) << 24;  // MSB-first
    ...
    // refout=true
    crc = reflect32(crc);
    // xorout=0x00000000 ⇒ keine Änderung
    return crc;
}
...
static float decode_value(uint8_t* b, size_t start_idx, size_t offs_hi) {
    ...
    if(b[start_idx - 1] == TFA_303905_PREAMBLE_ID) { 
	    val = (int16_t)(raw << 4) >> 4; // 12-Bit sign-extenden
    } else if(b[start_idx - 1] == TFA_303902_PREAMBLE_ID) {
	    val = (int16_t)(raw << 5) >> 5; // 13-Bit sign-extenden
    }
    return (float)val / 10.0f;
}

static int tfa_303905_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (bitpos >= MESSAGE_BITLEN) {
        print_logf(LOG_DEBUG, __func__, "message too long, skipping...");
        printf("message too long\n");
        return DECODE_ABORT_LENGTH;
    }

    bitbuffer_extract_bytes(bitbuffer, 0, bitpos, b, MESSAGE_BITLEN);
    ...
    printf("\n");
    */
    // check CRC32 checksum (residue = 0x0 when check succeeds)
    if(b[start_idx-1] == TFA_303902_PREAMBLE_ID) {
        if (crc32_reveng(&b[start_idx], 34, TFA_303902_CRC32_INIT)) {
            print_logf(LOG_DEBUG, __func__, "TFA 30.3902.02 CRC32 failed");
            return DECODE_FAIL_MIC;
        }
    } else if(b[start_idx-1] == TFA_303905_PREAMBLE_ID) {
        if (crc32_reveng(&b[start_idx], 35, TFA_303905_CRC32_INIT)) {
    ...
    int humidity, humidity_1, humidity_2            = 0.0f; // only required for TFA 30.3905
    float temp_c_ext, temp_c_ext_1, temp_c_ext_2    = 0.0f;
    ...
    switch(b[start_idx-1]) {
        case TFA_303905_PREAMBLE_ID:
            ...
            /* clang-format off */
            data = data_make(
                "model",                 "",                    DATA_STRING, "TFA-30.3905.02",
        ...
        case TFA_303902_PREAMBLE_ID:		
            ...
            /* clang-format off */
            data = data_make(
                "model",                 "",                    DATA_STRING, "TFA-30.3902.02",
                "id",                    "",                    DATA_STRING, id_str,
                "battery_ok",            "Battery OK",          DATA_INT,    !battery_low,
                "manual_transmit",       "Manual Transmit",     DATA_INT,    manual_transmit,
                "seq_number",            "Sequence Number",     DATA_INT,    seq,
...
r_device const tfa_303905 = {
        .name        = "TFA Dostmann 30.3905/02 T/H sensor with external sensor",
        .modulation  = FSK_PULSE_PCM,
        .short_width = 61,
        .long_width  = 61, // unused
        .tolerance   = 5, // noch anpassen auf z.b. 5
        .reset_limit = 1600,
        .decode_fn   = &tfa_303905_callback,
