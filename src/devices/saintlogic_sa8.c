/** @file
    Saintlogic SA8 Weather Station.
...
Data layout:
    ...
    Sample         46 54 24 cd ab 26 0c d0 bd c3 75 39 e3 39 e3 e8 44 f3 00 6f 00 3d 00 00 00 00 00 b4 00 7e 00 41 00 53 00 00 00 f1 10 17 1d
                   II II II II II II II II II II II II II II II CC CC CC CC TT TT HH 00 00 00 00 00 GG GG WW WW DD DD RR RR RR RR ?? ?? XX XX


- II:{120} Fixed value, ID / MAC address of the Outdor Weather Station, to be confirmed
- CC: {32} little endian LSB/MSB, Counter, +1 each message transmit
...
- RR: {32} little endian LSB/MSB, Rain Gauge in mm scale 0.15
- ??: {16} Unknown value, probably battery information here ?
- XX: {16} CRC or Checksum, not yet guessed

*/

static int saintlogic_sa8_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (len < 41) {
        decoder_log(decoder, 1, __func__, "Message too short");
        return DECODE_ABORT_LENGTH;
    }

    decoder_log_bitrow(decoder, 2, __func__, b, sizeof (b) * 8, "UART decoded MSG");
...
r_device const saintlogic_sa8 = {
        .name        = "Saintlogic SA8 Weather Station",
        .modulation  = OOK_PULSE_PCM,
        .short_width = 200,
        .long_width  = 200,
        .reset_limit = 2500,
        .decode_fn   = &saintlogic_sa8_decode,
