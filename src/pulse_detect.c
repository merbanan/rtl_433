...
void
pulse_detect_set_levels(pulse_detect_t *pulse_detect, int use_mag_est, float fixed_high_level, float min_high_level,
                        float high_low_ratio, int verbosity) {
...
int pulse_detect_package(pulse_detect_t *pulse_detect, int16_t const *envelope_data, int16_t const *fm_data, int len,
                         uint32_t samp_rate, uint64_t sample_offset, pulse_data_t *pulses, pulse_data_t *fsk_pulses,
                         unsigned fpdm) {
        int att_hist[37] = {0};
        int const samples_per_ms = samp_rate / 1000;
        pulse_detect_t *s = pulse_detect;
        s->ook_high_estimate = MAX(s->ook_high_estimate,
                                   pulse_detect->ook_min_high_level);    // Be sure to set initial minimum level
                ...
                switch (s->ook_state) {
                        case PD_OOK_STATE_IDLE:
                                if (am_n > (ook_threshold + ook_hysteresis)    // Above threshold?
                                    && s->lead_in_counter >
                                       OOK_EST_LOW_RATIO) { // Lead in counter to stabilize noise estimate
                                        // Initialize all data
                                        ...
                                        s->ook_low_estimate += ((ook_low_delta > 0) ? 1
                                                                                    : -1);    // Hack to compensate for lack of fixed-point scaling
                                        // Calculate default OOK high level estimate
                                        s->ook_high_estimate = pulse_detect->ook_high_low_ratio *
                                                               s->ook_low_estimate; // Default is a ratio of low level
                                        s->ook_high_estimate = MAX(s->ook_high_estimate,
                                                                   pulse_detect->ook_min_high_level);
                                        s->ook_high_estimate = MIN(s->ook_high_estimate, OOK_MAX_HIGH_LEVEL);
                                        if (s->lead_in_counter <= OOK_EST_LOW_RATIO)
                                                s->lead_in_counter++;        // Allow initial estimate to settle
                                }
                                break;
                        case PD_OOK_STATE_PULSE:
                                ...
                                if (am_n < (ook_threshold - ook_hysteresis)) {    // Gap?
                                        ...
                                        if (s->pulse_length < PD_MIN_PULSE_SAMPLES) {
                                                ...
                                                s->max_pulse = MAX(s->pulse_length,
                                                                   s->max_pulse);    // Find largest pulse
                                                s->pulse_length = 0;
                                        ...
                                        s->ook_high_estimate +=
                                                am_n / OOK_EST_HIGH_RATIO - s->ook_high_estimate / OOK_EST_HIGH_RATIO;
                                        s->ook_high_estimate = MAX(s->ook_high_estimate,
                                                                   pulse_detect->ook_min_high_level);
                                        s->ook_high_estimate = MIN(s->ook_high_estimate, OOK_MAX_HIGH_LEVEL);
                                ...
                                if (pulses->num_pulses == 0) {    // Only during first pulse
                                        if (fpdm == FSK_PULSE_DETECT_OLD)
                                                ...
                                                pulse_FSK_detect_mm(fm_data[s->data_counter], fsk_pulses,
                                                                    &s->FSK_state);
                                }
                                break;
                        case PD_OOK_STATE_GAP_START:    // Beginning of gap - it might be a spurious gap
                                ...
                                if (am_n > (ook_threshold + ook_hysteresis)) {    // New pulse?
                                        ...
                                        if (fsk_pulses->num_pulses > PD_MIN_PULSES) {
                                                ...
                                                if (pulse_detect->verbosity)
                                                        fprintf(stderr,
                                                                "Levels low: -%d dB  high: -%d dB  thres: -%d dB  hyst: (-%d to -%d dB)\n",
                                                                mag_to_att(s->ook_low_estimate),
                                                                mag_to_att(s->ook_high_estimate),
