...
static THREAD_RETURN THREAD_CALL accept_thread(void *arg) {
        ...
        for (;;) {
                ...
                srv->client_count += 1;
                unsigned prev_cnt =
                        srv->data_cnt + 9; // data sent in previous loop, random value to get the current buffer
                pthread_mutex_unlock(&srv->lock);
...
static int
rtltcp_server_start(rtltcp_server_t *srv, char const *host, char const *port, r_cfg_t *cfg, struct raw_output *output) {
        if (!host || !port)
                return -1;
