/** @file
    ...
    Additional assistance from zuckschwerdt
...
static int maverick_xr30_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (bitbuffer->bb[0][0] == 0x55) {
        uint8_t b[12];
        bitbuffer_extract_bytes(bitbuffer, 0, 7, b, 96); // shift in case first bit was not received properly
    } else if (bitbuffer->bb[0][0] == 0xaa) {
        uint8_t b[12];
        bitbuffer_extract_bytes(bitbuffer, 0, 8, b, 96); // shift in case first bit was received properly
    } else {
