...
{
    // Available in Windows 7 or later (32-bit or 64-bit)
    // 32-bit Windows XP or earlier should use GetTickCount(), wraps every 49.7 days
    // ULONGLONG GetTickCount64(); // has ~15 ms resolution

    *interval_timer = GetTickCount64(); // number of milliseconds that have elapsed since the system was started.
}
...
{
    // this clock has only ~15-ms precision on win32, consider QueryPerformanceCounter() if needed
