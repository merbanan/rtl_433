...
static int ambientweather_tx8300_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (74 != bitbuffer->bits_per_row[0]) {
        if (decoder->verbose > 1)
            decoder_logf(decoder, 0, __func__, "AmbientWeather-TX8300: wrong size (%u bits)", bitbuffer->bits_per_row[0]);
    ...
    if (decoder->verbose > 1)
        decoder_logf(decoder, 0, __func__, "H: %02x, F:%02x", b[0], b[1] & 0xc0);
