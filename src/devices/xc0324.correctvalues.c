//
// Functions to generate reference value csv files when run with -DDD
// ie with debug_output >= 3
//

static int
xc0324_values2csv(bitbuffer_t *bitbuffer, unsigned row, unsigned bitpos, data_t ** data)
{   // Working buffers
...
static int xc0324_referenceValues2csv(bitbuffer_t *bitbuffer)
{
    ...
    for (r = 0; r < bitbuffer->num_rows; ++r) {
        ...
        while ((bitpos = bitbuffer_search(bitbuffer, r, bitpos,
            ...
            events += result = xc0324_values2csv(bitbuffer, r, bitpos, &data);
            if (result) {
              return events; // break after first successful message
            }
