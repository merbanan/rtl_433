...
The `rtl_433 -a -t -D -D` output correctly guesses the older transmitter modulation
...
* [A good helper, in French](https://enavarro.me/ajouter-un-decodeur-ask-a-rtl_433.html)
...
It could have been **much** quicker if I'd known what I needed to get from
each experiment :-)
...
these parameters from the `rtl_433 -a` output.

```
Test mode active. Reading samples from file: g011_433.92M_250k.cu8
Input format: CU8 IQ (2ch uint8)
*** signal_start = 2470, signal_end = 65537
signal_len = 63067,  pulses = 150
Iteration 1. t: 102    min: 90 (3)    max: 114 (147)    delta 40
Iteration 2. t: 102    min: 90 (3)    max: 114 (147)    delta 0
Distance coding: Pulse length 102

Short distance: 129, long distance: 158, packet distance: 251
```

Well you can once you know that the signal was sampled at 250KHz, so 1
sample unit lasts (1/ 250000) seconds = 4 us (microseconds), so, for
example, a pulse of length 102 sampled units lasts for 102 * 4 us.
...
approach (think UDP if you known about LAN network protocols), rather
...
again.  First note that with `0x5F` as the sync bit, we are (in most
cases) now finding 3 identical repeats of the message in a single row of
the bitbuffer.  (That was far from obvious before, because the 2 bit gap
...
byte level parity checks, otherwise it would have take me **much**
...
noticed that there was a sequential pattern in the values of byte 5.
