...
static int thermopro_tp211b_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    // Checksum
    uint16_t checksum = b[6] << 8 | b[7];
    uint16_t checksum_calc = 0x411b; // final XOR can be put here.

    for (int n = 0; n < 6; n++) {
        for (int i = 0; i < 8; i++) {
            int bit = (b[n] << (i + 1)) & 0x100;
            if (bit) {
                checksum_calc ^= xor_table[(n * 8) + i];
            }
        }
    }

    if (checksum != checksum_calc) {
        decoder_logf(decoder, 2, __func__, "Checksum error, calculated %04x, expected %04x", checksum_calc, checksum);
