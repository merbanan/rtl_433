  ...
  if( ((bb[0][1] == 0x55) && (bb[0][2] == 0x55)) ||
    ...
    if ((sensor_id == ID_THGR122N) || (sensor_id == ID_THGR968)){
      ...
      if(debug_output >= 1) {
        ...
        fprintf(stderr, "Message: "); for (i=0 ; i<20 ; i++) fprintf(stdout, "%02x ", msg[i]); fprintf(stdout,"\n");
      }
    } else {
      if(debug_output >= 1) {
...
return 1;
      ...
      if(debug_output >= 1) {
        ...
        fprintf(stderr, "Raw Data: "); for (i=0 ; i<BITBUF_COLS ; i++) fprintf(stdout, "%02x ", bb[0][i]); fprintf(stdout,"\n\n");
      }
    }
  }
