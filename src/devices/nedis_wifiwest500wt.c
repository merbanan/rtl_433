/** @file
    ...
    @sa https://github.com/make-all/tuya-local/discussions/4136

    Manufacturer : Fujian Youtong Industries Co., Ltd. ??
    Sold as      : Nedis WIFIWEST500WT (Tuya-compatible)
    ...
    Capture test:
        ...
        rtl_433 -f 868.4M -s 250k -Y classic \
            -X "n=nedis_wifiwest500wt,m=FSK_PCM,s=122,l=122,r=8000"

    --- Packet Structure (17 bytes) ---

        [0]      0xF0            Fixed header byte
        [1]      0x5N            N = 3-bit rolling counter (0-7)
        [2..4]   xx xx xx        24-bit device ID (unique per sensor unit)
        [5..6]   TT TT           Temperature (12-bit, bits 0-3 of [5] + all of [6]).
                                 Format is (temp_c * 10). Two's complement for < 0.
        [7]      0xHH            Humidity (%RH)
        [8]      0xAA            Wind speed average LSB
        [9]      0xDA            D = Average Wind Direction (high nibble), A = Wind speed average MSB (low nibble)
        [10]     0xGG            Wind gust LSB
        [11]     0xDG            D = Gust Wind Direction (high nibble), G = Wind gust MSB (low nibble)
        [12..13] RR RR           Rain counter (16-bit little-endian), 0.35 mm per tip
        [14]     0x00            Padding / Reserved
        [15]     CK              Checksum byte (proprietary algorithm)
        [16]     ~CK             Ones-complement of CK; CK + ~CK == 0xFF always
*/
 ...
 * The checksum byte and its ones-complement must sum to 0xFF.
 */
...
r_device const nedis_wifiwest500wt = {
    ...
    .short_width    = 122,     // chip width in µs
